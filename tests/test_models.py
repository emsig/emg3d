...
class TestModel:
    ...
    def test_interpolate(self):
        ...
        res_z = res_x*1.4
        mu_r = res_x*1.11
        epsilon_r = res_x*3.33

        model1inp = models.Model(grid, res_x)

        model1out = model1inp.interpolate2grid(grid, grid2)
        assert_allclose(model1out._res_x[0],
                        10**(np.sum(np.log10(model1inp.res_x))/8))
        assert model1out._res_y is None
        assert model1out._res_z is None
        assert model1out._mu_r is None
        assert model1out._epsilon_r is None

        model2inp = models.Model(grid, res_x=res_x, res_y=res_y, res_z=res_z,
                                 mu_r=mu_r, epsilon_r=epsilon_r)

        model2out = model2inp.interpolate2grid(grid, grid2)
        assert_allclose(model2out._res_x[0],
                        10**(np.sum(np.log10(model2inp.res_x))/8))
        assert_allclose(model2out._res_y[0],
                        10**(np.sum(np.log10(model2inp.res_y))/8))
        assert_allclose(model2out._res_z[0],
                        10**(np.sum(np.log10(model2inp.res_z))/8))
