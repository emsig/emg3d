...
__all__ = ['solve', 'solve_source', 'Solver', 'multigrid', 'krylov',
           'smoothing', 'restriction', 'prolongation', 'residual',
           'MGParameters', 'RegularGridProlongator']
...
class Solver:
    ...
    def _check_grid(self, grid_tuple):
        ...
        if any(nC not in good_numbers for nC in grid.shape_cells):
            print(f"Warning: mesh dimension {(grid.shape_cells)} is not "
                  f"optimal for MG solver. Good numbers are: {good_numbers}")
    ...
    def _set_map_process_list(
        ...
        # Create source fields if ndarrays (vectors) where provided.
        ...
        return list(zip(model_list, sfield_list))
